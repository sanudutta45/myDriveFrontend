...
export const addFolder = async (values) => {
  const url = ss_login_page_api_url + `/api/v1/folder?id=${values.parentId}`
  const reqObj = {
    name: values.name,
...
export const renameFolder = async (values) => {
  const url = ss_login_page_api_url + "/api/v1/folder/rename/" + values.folderId
  const reqObj = {
    newName: values.newName,
  }
  try {
    const result = await apiCalls("put", url, reqObj)
    return result.data
  } catch (error) {
    if (error.response) {
      throw new Error(error.response.data.error)
    } else {
      console.error("rename folder, server err:", error.message)
      throw new Error(error.message)
    }
  }
}

export const deleteFolder = async (values) => {
  const url = ss_login_page_api_url + "/api/v1/folder/delete/" + values.folderId
  ...
  try {
    ...
    if (error.response) {
      ...
      console.error("delete folder, server err:", error.message)
      throw new Error(error.message)
    }
  }
}
